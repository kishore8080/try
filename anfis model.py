    ...
    def fit(self, X_train, y_train, X_test=None, y_test=None, optimize_test_data=False):
        ...
        for gen in range(generations):
            ...
            for i in range(len(popParam)):
                L1, L2, L3, L4 = self.forwardPass(popParam[i], X_train, inputsTrain, AntTrain, L1Train, L2Train,
                                                  functions)
                pred_train, Trained_models = self.linear_fit(L3, L4, X_train, y_train, functions, models)
                mse_train = self.rmse(y_train, pred_train)

                if optimize_test_data:
                    L1, L2, L3, L4 = self.forwardPass(popParam[i], X_test, inputsTest, AntTest, L1Test, L2Test,
                                                      functions)
...
data = pd.read_excel('stroke data.xls')
Data = data.loc[:,['heart_disease','hypertension','age','avg_glucose_level','bmi','stroke']]
aa = Data['stroke']
minima = aa.min()
maxima = aa.max()
...
X_train = data_train.drop("stroke",axis=1).values
y_train = data_train["stroke"].copy().values
X_test = data_test.drop("stroke",axis=1).values
y_test = data_test["stroke"].copy().values
...
#a=[i for i in range(1,len(pred_train)+1)]
#b=[i for i in range(1,len(y_train)+1)]
#plt.scatter(a,pred_train,c="red",label="pred_train")
#plt.scatter(b,y_train,c="blue",label='y_train')
#plt.title("yTrain vs yPredection")
#plt.show()
...
from scipy.stats import pearsonr
#pearsonr((y_train,float),(pred_train.reshape(-1,1),float))
from sklearn.metrics import mean_absolute_error
print(mean_absolute_error(y_train,pred_train))

#with open('bestParam.pkl', 'wb') as files:
 #    pickle.dump(bestParam, files)

#with open('bestModel.pkl', 'wb') as files:
 #     pickle.dump(bestModel, files)
#filename = 'model.sav'
